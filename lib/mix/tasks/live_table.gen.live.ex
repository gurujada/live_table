if Code.ensure_loaded?(Igniter.Mix.Task) do
  defmodule Mix.Tasks.LiveTable.Gen.Live do
    ...
    ## Usage
        ...
        $ mix live_table.gen.live Context Schema table_name field:type ...

    Example:

        $ mix live_table.gen.live Accounts User users name:string email:string
    ...
    For manual setup, generate with `mix phx.gen.live` and wire LiveTable manually.
    """
      ...
      |> then(fn igniter ->
        case Igniter.Project.Module.find_and_update_module(igniter, index_module, fn zipper ->
          ...
          {:error, igniter} ->
            Igniter.add_warning(
              igniter,
              "Could not find or update module #{inspect(index_module)}"
            )
        end
      end)
    end
  ...
  defmodule Mix.Tasks.LiveTable.Gen.Live do
    ...
    Or generate with `mix phx.gen.live` and wire LiveTable manually.
    """

    use Mix.Task

    @shortdoc "Generates LiveView files with LiveTable (requires :igniter)"

    @impl Mix.Task
    def run(_argv) do
      Mix.raise("""
      mix live_table.gen.live requires the :igniter dependency.
